...
def naiveSoftmaxLossAndGradient(
    ...
    x = outsideVectors.dot(centerWordVec)
    x = softmax(x)
    loss = -np.log( x[outsideWordIdx] )

    gradCenterVec = -outsideVectors[outsideWordIdx] + outsideVectors.T.dot(x)

    gradOutsideVecs =  x.reshape((x.shape[0],1)).dot(centerWordVec.reshape((centerWordVec.shape[0],1)).T)
...
def negSamplingLossAndGradient(
    ...
    matrix = outsideVectors[indices]
    matrix[1:] = -1.0*matrix[1:]

    sigmoid_mat = sigmoid(matrix.dot(centerWordVec))

    loss = np.sum(-np.log(sigmoid_mat))

    sigmoid_mat = (sigmoid_mat - 1.0)

    gradCenterVec = matrix.T.dot(sigmoid_mat)

    gradOutsideVecs[outsideWordIdx] += sigmoid_mat[0]*centerWordVec
    gradOutsideVecs[negSampleWordIndices] += np.outer(sigmoid_mat[1:],centerWordVec)*-1
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    currentCenterVec = centerWordVectors[currentCenterWordIdx]

    for outsideword in outsideWords:
        outsideWordidx = word2Ind[outsideword]
        currloss, currgradcenter, currgradoutside = word2vecLossAndGradient(currentCenterVec, outsideWordidx, outsideVectors, dataset)
        loss += currloss
        gradCenterVecs[currentCenterWordIdx] += currgradcenter
