...
def svm_loss(x, y):
    ...
    #What we know about loss gradient is its the shape of (50,10)
    N = x.shape[0]
    correct_class_scores = x[np.arange(N),y].reshape(N,1)
    margin = np.maximum(0,x-correct_class_scores+1)
    margin[np.arange(N), y] = 0
    loss = margin.sum()/N
    num_pos = np.sum(margin>0,axis=1) # count for how many elements margin is greater than 0
    dx = np.zeros_like(x) # num_pos time the syi appeared
    dx[margin > 0] = 1
...
def softmax_loss(x, y):
    ...
    #loss = 0.0
    #dx = np.zeros_like(x)
    # though works without - np.max(x, axis=1, keepdims=True)
    #soft_row_sum = (np.exp(x).sum(axis=1, keepdims = True))
    #softmax = np.exp(x)/ soft_row_sum
